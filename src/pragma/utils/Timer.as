...
package pragma.utils
{
	 ...
	 */
	public class Timer extends EventDispatcher implements $ILoopEntry
	{
		...
		public function notifyRegister($loop:Loop):void {
			////Logger.debug(this,"Timer::notifyRegister");
			this.addEventListener(TimerEvent.TIMER_COMPLETE,onTimerCompleteHandler);
			this.addEventListener(Event.CLOSE,onTimerCompleteHandler);
		}
		private function onTimerCompleteHandler($e:Event):void {
			loop.pragma::removeEntry(this);
		}
		public function notifyUnregister($loop:Loop):void {
			////Logger.debug(this,"Timer::notifyUnregister");
			this.removeEventListener(TimerEvent.TIMER_COMPLETE,onTimerCompleteHandler);
			this.removeEventListener(Event.CLOSE,onTimerCompleteHandler);
		}
		public function heartbeat ():Boolean {
			if( this.running ) {
				this._elapsed = Program.getTimer() - this._started;
				if( this._elapsed >= (this.delay * (this.currentCount+1)) ) {
					this.tick();   
				}
			}
			return this.running;
